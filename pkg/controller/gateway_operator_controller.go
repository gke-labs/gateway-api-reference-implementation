...
type GatewayOperatorReconciler struct {
	GatewayControllerOptions
}

func (r *GatewayOperatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
...
func (r *GatewayOperatorReconciler) syncInfrastructure(ctx context.Context, gw *gatewayv1.Gateway) (*corev1.Service, error) {
	...
	// 2. Create/Update ClusterRoleBinding for the ServiceAccount
	crb := &rbacv1.ClusterRoleBinding{
		ObjectMeta: metav1.ObjectMeta{
			Name: fmt.Sprintf("gari-gateway-%s-%s", gw.Namespace, gw.Name),
		},
	}
	// We can't set controller reference on cluster-scoped resources to namespace-scoped resources.
	// We'll have to manage cleanup manually or just leave it for now.
...
func (r *GatewayOperatorReconciler) SetupWithManager(mgr ctrl.Manager) error {
